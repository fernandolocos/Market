...
public class CadastroMain {
	...
	private void excluirProduto() {
		Produto prod = pesquisaProduto(Dialogo
				.pegaValorInt("Digite o id do Produto"));
	...
	private void cadastraUnidade() {
		try {
			...
			String descricao = Dialogo
					.pegaValor("Digite a descrição da unidade:");
	...
	private void cadastraProduto() {
		try {
			...
			long qtdeminima = Dialogo
					.pegaValorLong("Quantidade mínima em estoque:");
	...
	private Unidade pesquisaUnidade(String sigla) {
		...
		return null;
		/*
		 * Unidade un = new Unidade(); un.setId(1); un.setSigla("CX");
	...
	private void pesquisaCliente() {
		...
		if (cliente != null) {
			...
			if (cliente.getTipo() == TipoPessoa.Juridica) {
