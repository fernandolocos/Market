		...
		do {
			...
			if (bip != null) {
				if (bip.toLowerCase().contains("x")) {
					...
					qtdeBip = Integer.parseInt(bipSep[0]); // valor antes do
															// primeiro x é a
															// qtde

					if (bipSep.length == 2) {
						prodBip = Long.parseLong(bipSep[1]); // valor depois de
																// x é o código
																// do produto
					} else if (bipSep.length == 3) {
						descBip = Double.parseDouble(bipSep[1]); // valor
																	// central é
																	// o
																	// desconto
						prodBip = Long.parseLong(bipSep[2]); // valor depois do
																// segundo x é o
																// código do
																// produto
					}
				} else {
					prodBip = Long.parseLong(bip); // sem é apenas o produto com
													// a qtde 1
				}
